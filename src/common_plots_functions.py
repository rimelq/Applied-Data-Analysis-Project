...
#   This script contains the code for the basic interactive plots that are we made for 
#   for each region (ethnicity comparison, age distribution, gender proportions).
#   Due to issue with size and compatibility with the site, we later on made the plots with flourish 
...
def generate_flourish_csv_for_all_regions(region_data, output_file="ethnicity_comparison_all.csv"):
    # Initialize an empty list to hold data
    all_data = []
    
    # Loop through all regions
    for region, datasets in region_data.items():
        # Extract the datasets for the region
        data_ethnicity = datasets['data_ethnicity']
        realworld_ethnicity = datasets['realworld_ethnicity']
        ethnicities = datasets['ethnicities']
        ...
        for period in periods:
            # Calculate population ethnicities
            if period == "All periods":
                population_ethnicities = realworld_ethnicity.groupby('group')['size'].mean()
            else:
                population_ethnicities = realworld_ethnicity[
            ...
            genres = ['All'] + sorted(data_ethnicity['main_genre'].dropna().unique())
            for genre in genres:
                # Calculate actor ethnicities
                def calculate_weighted_actor_ethnicities(df, period, genre, ethnicities):
                    df = df.copy()
                    if period != "All periods":
                        start_year, end_year = map(int, period.split('-'))
                ...
                region_data_row = pd.DataFrame({
                    "Region": region,
                    "Period": period,
                    "Genre": genre,
                    "Ethnicity Group": population_ethnicities.index,
                    "Real-world Population (%)": population_ethnicities.values * 100,
                    f"{region} Industry (%)": actor_ethnicities.values * 100
                })
                all_data.append(region_data_row)

    # Concatenate all data and save to CSV
    all_data_combined = pd.concat(all_data, ignore_index=True)
    all_data_combined.to_csv(output_file, index=False)
    print(f"CSV file for all regions saved to {output_file}")


##########################################################


def save_age_distribution_to_csv(output_file="age_distribution.csv"):
    data_list = []
    
    # Iterate over regions, genders, periods, and genres
    for region in regions:
        ...
        for gender in ['Male', 'Female', 'Both']:
            for period, (start_year, end_year) in {
                ...
                for genre in ['All'] + sorted(datasets['region_data']['main_genre'].dropna().unique()):
                    # Get real-world density
                    real_density, age_range = compute_real_density(
                        datasets, gender, period
                    )
                    if real_density is None:
                        continue
                    
                    # Get actor density
                    actor_density = compute_actor_density(
                        datasets, start_year, end_year, gender, genre
                    )
                    if actor_density is None:
                        continue

                    # Combine data for the CSV
                    for age, real_d, actor_d in zip(age_range, real_density, actor_density):
                        data_list.append({
                            "Region": region,
                            "Gender": gender,
                            "Period": period,
                            "Genre": genre,
                            "Age Group": f"{int(age)}-{int(age)+10}",
                            "Real Density": real_d,
                            "Actor Density": actor_d
                        })
...
def compute_actor_density(datasets, start_year, end_year, gender, genre):
    # Get the actor dataset for the selected region
    df_actors = datasets['region_data'].copy()
    ...
    # Extract actor ages and filter
    actor_ages = df_actors['age_at_release'].dropna().astype(float)
    actor_ages = actor_ages[(actor_ages >= 0) & (actor_ages <= 100)]  # Filter valid age range

    if actor_ages.empty:
        print("No actor age data available for the selected filters.")
        return None

    # Generate KDE for actor ages
    actor_kde = gaussian_kde(actor_ages)
    age_range = np.linspace(0, 100, 500)  # Define a consistent age range
    actor_density = actor_kde(age_range)
    actor_density /= actor_density.sum()  # Normalize

    return actor_density