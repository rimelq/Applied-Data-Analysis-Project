...
def age_distribution_plot(europe_avg_male, europe_avg_female, europe_df, time_periods, period="All Periods", genre="All Genres"):
    ...
    df_actors['release'].dropna(inplace=True)
...
def gender_OLS(df):
    # take only years above 1950
    df = df[df['release'] >= 1950]

    # Group by year and count gender per year
    gender_counts = df.groupby(['release', 'actor_gender']).size().unstack(fill_value=0).reset_index()
    ...
    # Fit OLS
    model = smf.ols(formula="female_prop ~ release", data=gender_counts).fit()
    ...
    plt.scatter(gender_counts['release'], gender_counts['female_prop'], color='blue', label='Actual Female Proportion')
    plt.plot(gender_counts['release'], gender_counts['predicted_prop'], color='red', linewidth=2, label='Trend Line (Linear Regression)')
    plt.xlabel('Year of Release')
    plt.ylabel('Proportion of Female Actors')
    plt.title('Gender Ratio Over Time in European Cinema')
    plt.legend()
    plt.show()

    print(f"Linear Regression Coefficients:")
    print(f"Intercept: {model.params['Intercept']}")
    print(f"Year Coefficient: {model.params['release']}")


###############################################################

def radar_chart_plot(df, ethnicities):
...
def calculate_gender_proportions(df, time_periods, genres):
    ...
    for period_name, (start_year, end_year) in time_periods.items():
        df_period = df[df['release'].between(start_year, end_year)]
